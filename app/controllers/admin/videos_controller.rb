class Admin::VideosController < AdminController
  ...
  def create
    @video = VideoBuilder.default_with(video_params)
    @video = VideoBuilder.read_source_dimensions(@video)
    if @video.save
  ...
  def update
    @video = QueryVideos.new(params: params).find
    @video = VideoBuilder.read_source_dimensions(@video)
